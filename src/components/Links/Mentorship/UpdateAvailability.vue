...
export default {
  ...
  methods: {
    async update() {
      ...
      const snapshot = await mentorAvailabilityRef.doc(docId).get();

      await mentorAvailabilityRef.doc(docId).set(
        {
      ...
      const activity = {
        ...
        statement: `Mentor ${data.mentor.name} updated their availability to ${this.date} at ${this.time}`,
        createdAt: new Date(),
        type: "mentor",
        photoURL: data.mentor.photoURL,
        notification: true,
        read: false,
      };
      ...
      (this.date = null), (this.time = null), (this.info = null);
    },
  },
};
</script>
